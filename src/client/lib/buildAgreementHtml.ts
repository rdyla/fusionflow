...
type CustomLine = OppFormData["customLines"][number];

function lineDisplayLabel(l: CustomLine): string {
  const base = l.label || (l.kind === "discount_percent" || l.kind === "discount_amount" ? "Discount" : "Line item");
  if (l.kind === "discount_percent") return `${base} (${Number(l.price) || 0}% off)`;
  return base;
}

function lineNoteText(l: CustomLine): string {
  if (l.kind === "discount_percent") return "Percentage discount";
  if (l.kind === "discount_amount")  return "Discount";
  return "Custom line item";
}

/** Annual total before any custom lines — needed so discount_percent lines can
 *  compute their dollar effect. */
function preCustomAnnualFrom(calc: OppCalcResult): number {
...
function pricingScheduleHtml(calc: OppCalcResult, term: number): string {
  ...
  return `
    <div style="margin:14px 0 4px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#0d1b2e;">Annual Pricing Schedule</div>
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:4px;">
      <tr>
        <th style="padding:5px 10px;text-align:left;border-bottom:2px solid #0d1b2e;font-size:11px;text-transform:uppercase;letter-spacing:0.04em;color:#64748b;">Term Year</th>
        <th style="padding:5px 10px;text-align:right;border-bottom:2px solid #0d1b2e;font-size:11px;text-transform:uppercase;letter-spacing:0.04em;color:#64748b;">Annual</th>
        <th style="padding:5px 10px;text-align:right;border-bottom:2px solid #0d1b2e;font-size:11px;text-transform:uppercase;letter-spacing:0.04em;color:#64748b;">Discount</th>
        <th style="padding:5px 10px;text-align:right;border-bottom:2px solid #0d1b2e;font-size:11px;text-transform:uppercase;letter-spacing:0.04em;color:#64748b;">Net</th>
      </tr>
      ${rows}
      <tr>
        <td colspan="3" style="padding:7px 10px;text-align:right;font-weight:700;color:#0d1b2e;border-top:2px solid #0d1b2e;">Total Contract Value</td>
        <td style="padding:7px 10px;text-align:right;font-weight:800;color:#0d1b2e;border-top:2px solid #0d1b2e;">${fmtFull(calc.tcv)}</td>
      </tr>
    </table>`;
}
  ...
  return `
    <div class="mso-section" style="border-top:3px solid #00b8a0;margin-top:30px;padding-top:22px;">
      ...
      <div class="mso-sla-table" style="margin-bottom:14px;">
        ...
        <thead><tr style="background:#0d1b2e;color:#fff;">
...
export function buildProposalHtml(_oppName: string, d: OppFormData, calc: OppCalcResult, versionNum: number): string {
  ...
  const proposalPreCustom = preCustomAnnualFrom(calc);
  const customSection = (d.customLines && d.customLines.length > 0) ? `
      ...
      <div class="pricing-wrap">
        <table class="pricing-table">
          ...
          <tbody>
            ${d.customLines.map(l => {
              const effect = customLineDollar(l, proposalPreCustom);
              return `<tr><td class="label-cell">${escHtml(lineDisplayLabel(l))}</td><td class="price-col">${fmtSigned(effect)}</td></tr>`;
            }).join("")}
          </tbody>
        </table>
      </div>
...
<div class="agreement-doc">
  ...
  <div class="doc-body">
    ...
    <div class="coverage-grid">
      <div class="coverage-card"><div class="coverage-card-title">Adds, Moves &amp; Changes During Business Hours</div><div class="coverage-card-desc">Provided at no additional charge from 7:00 am \u2013 5:00 pm PST. Timely updates with no extra billing.</div></div>
      <div class="coverage-card"><div class="coverage-card-title">After-Hours Support at ${fmtFull(afterHours)}/hr</div><div class="coverage-card-desc">Discounted labor for engagements over 30 minutes outside standard business hours \u2014 ideal for multi-timezone operations.</div></div>
      <div class="coverage-card"><div class="coverage-card-title">Advanced Task Rate at ${fmtFull(advRate)}/hr</div><div class="coverage-card-desc">Preferred pricing for new call flows, advanced user setups, third-party integrations, and API or custom development.</div></div>
      <div class="coverage-card"><div class="coverage-card-title">Proactive SLA Guarantees</div><div class="coverage-card-desc">Standard requests responded to within 4 hours. Critical or emergency situations receive a 30-minute response commitment.</div></div>
      <div class="coverage-card"><div class="coverage-card-title">Packet Fusion Customer Portal Access</div><div class="coverage-card-desc">Submit and track support tickets, review historical case records, and receive real-time status updates in one place.</div></div>
    ...
    <div class="term-panel">
      <div class="term-dates">
        <div class="term-date-cell"><div class="term-date-label">Start Date</div><div class="term-date-val">${startDate}</div></div>
        <div class="term-date-cell"><div class="term-date-label">End Date</div><div class="term-date-val">${endDate}</div></div>
        <div class="term-date-cell"><div class="term-date-label">Term Length</div><div class="term-date-val">${term} Year${term > 1 ? "s" : ""}</div></div>
      </div>
    ...
    <div class="price-summary"${calc.totalDiscount > 0 ? ` style="grid-template-columns:1fr 1px 1fr 1px 1fr;"` : ""}>
      ...
      <div class="price-summary-cell">
        <div class="ps-label">Promotional Discounts</div>
        <div class="ps-value" style="color:#16a34a;font-size:22px;">\u2212${fmtFull(calc.totalDiscount)}</div>
        <div class="ps-sub">Applied per schedule below</div>
      </div>
      <div class="price-summary-divider"></div>` : ""}
      <div class="price-summary-cell">
    ...
    ${pricingScheduleHtml(calc, term)}
  </div>
  <div class="sig-band">
    <div class="sig-band-title">Authorization &amp; Agreement</div>
    <div class="sig-grid">
      <div><div class="sig-party-name">Packet Fusion, Inc.</div>${docSigRows()}</div>
      <div><div class="sig-party-name">${escHtml(customer)}</div>${docSigRows()}</div>
    </div>
  </div>
</div>
</body>
...
export function buildSignatureHtml(_oppName: string, d: OppFormData, calc: OppCalcResult, versionNum: number): string {
  ...
  const sigPreCustom = preCustomAnnualFrom(calc);
  const customLineItems = (d.customLines && d.customLines.length > 0)
    ? d.customLines.map(l => {
      const effect = customLineDollar(l, sigPreCustom);
      const isDiscount = (l.kind ?? "charge") !== "charge";
      return `
...
<div class="sig-doc">
  ...
  <div class="sd-body">
    ...
    <div class="sd-totals">
      ...
      ${calc.totalDiscount > 0 ? `<div class="sd-total-row"><span class="sd-total-label">Promotional Discounts</span><span class="sd-total-val" style="color:#16a34a;">−${fmtFull(calc.totalDiscount)}</span></div>` : ""}
    ...
    <div class="sd-scope">
      ...
      <div style="display:flex;flex-direction:column;gap:7px;">
        <div style="display:flex;align-items:flex-start;gap:10px;font-size:12.5px;color:#374151;"><span style="width:18px;height:18px;border-radius:50%;background:#f1f5f9;border:1px solid #e2e8f0;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#94a3b8;flex-shrink:0;margin-top:1px;">1</span><div><strong style="color:#1e293b;">Major Call Flow Overhauls</strong> \u2014 Significant structural redesign of existing call flow configurations.</div></div>
    ...
    <div class="sd-sig-section">
      ...
      <div class="sd-sig-grid">
        <div><div class="sd-sig-party-label">Service Provider</div><div class="sd-sig-party-name">Packet Fusion, Inc.</div>${sdSigFields()}</div>
        <div><div class="sd-sig-party-label">Customer</div><div class="sd-sig-party-name">${escHtml(customer)}</div>${sdSigFields()}</div>
      </div>
      <div class="sd-footer">
