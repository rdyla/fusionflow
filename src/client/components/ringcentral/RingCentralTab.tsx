...
function Step({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
      ...
      <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.6, paddingTop: 2 }}>{children}</div>
    </div>
  );
}
...
function Code({ children }: { children: React.ReactNode }) {
  return (
    <code style={{ background: "rgba(0,0,0,0.07)", padding: "1px 5px", borderRadius: 3, color: "#1e293b", fontSize: 12 }}>
      {children}
    </code>
  );
}
  ...
  return (
    <div style={{ display: "grid", gap: 20, maxWidth: 680 }}>
      <div className="ms-section-card">
        ...
        <div style={{
          background: "#f8fafc", borderRadius: 6, padding: "16px 18px",
          border: "1px solid rgba(0,0,0,0.07)", display: "grid", gap: 14,
        }}>
          <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#94a3b8" }}>
            Setup steps (in the customer's RingCentral admin portal)
          </div>

          <Step n={1}>
            Sign in to <strong style={{ color: "#334155" }}>developers.ringcentral.com</strong> using the customer's admin credentials.
          </Step>
          <Step n={2}>
            Click <strong style={{ color: "#334155" }}>Create App</strong> → choose <strong style={{ color: "#334155" }}>REST API App</strong>, then set Auth type to <strong style={{ color: "#334155" }}>JWT auth flow</strong>.
          </Step>
          <Step n={3}>
            Under <strong style={{ color: "#334155" }}>OAuth Scopes</strong>, add the following:
          ...
          <Step n={4}>
            Go to <strong style={{ color: "#334155" }}>Credentials</strong> and generate a <strong style={{ color: "#334155" }}>JWT token</strong> for a super-admin user. Copy the <Code>Client ID</Code>, <Code>Client Secret</Code>, and <Code>JWT Token</Code>.
          </Step>
          <Step n={5}>
