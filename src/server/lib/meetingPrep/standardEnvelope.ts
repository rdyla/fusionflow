...
export function renderStandard(data: StandardMeetingPrepData): { subject: string; html: string } {
  ...
  const labelHtml = data.label && data.label.trim()
    ? `<div style="margin:0 0 18px;font-size:13px;color:#22c55e;font-weight:600;letter-spacing:0.04em;">${escapeHtml(data.label.trim())}</div>`
    : "";
  ...
  const html = base(`
    <h2 style="margin:0 0 6px;font-size:20px;font-weight:700;color:#22c55e;">${escapeHtml(data.title)} &mdash; ${projectName}</h2>
    ${labelHtml}
