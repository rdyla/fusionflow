...
app.post("/:projectId/zoom/recordings/confirm", async (c) => {
  ...
  for (const conf of parsed.data.confirmations) {
    ...
    await c.env.DB
      .prepare(`
        INSERT INTO zoom_recordings (id, project_id, phase_id, meeting_id, topic, start_time, duration_mins, host_email, recording_files, match_reason, manually_assigned)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
      ...
      `)
      .bind(id, projectId, conf.phase_id ?? null, conf.meeting_id, conf.topic, conf.start_time, conf.duration_mins, conf.host_email ?? null, JSON.stringify(conf.recording_files), conf.match_reason ?? null)
      .run();

    const row = await c.env.DB
      .prepare(`
        SELECT zr.*, ph.name AS phase_name
        FROM zoom_recordings zr LEFT JOIN phases ph ON ph.id = zr.phase_id
        WHERE zr.project_id = ? AND zr.meeting_id = ? LIMIT 1
      `)
...
app.patch("/:projectId/zoom/recordings/:recordingId", async (c) => {
  ...
  const { phase_id } = await c.req.json() as { phase_id: string | null };

  await c.env.DB
    .prepare("UPDATE zoom_recordings SET phase_id = ?, manually_assigned = 1, match_reason = 'manual', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND project_id = ?")
    .bind(phase_id ?? null, recordingId, projectId)
    .run();
